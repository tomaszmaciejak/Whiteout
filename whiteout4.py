...
def find_image(images, debug_name="", threshold=0.8, important=False, cnt_tries=5):
    ...
    # Take a screenshot
        for i in range(cnt_tries):
            ...
            screenshotB, screenshotG, screenshotR = cv2.split(screenshot)
            ...
            if debug>0 or important:
                ...
                if max_val >= threshold:
                    t=t+" OK"
                    print("R="+str(resultR[y,x])+" G="+str(resultG[y,x])+" B="+str(resultB[y,x]))
 ...
 #   time.sleep(2*wait_time)
    ...
    while True:
        if find_and_click((radar_img,location_radar_img), "  radar") :
            ...
            '''if (not "radar_tent" in character_state[player_str] or character_state[player_str]["radar_tent"] > datetime.now() + timedelta(minutes=15)) and '''
            if find_and_click((radar_tent_blue_img, radar_tent_gold_img, radar_tent_purple_img), "  radar tent", 0.9):
                ...
                if find_and_click((radar_swords_blue_img, radar_swords_purple_img,radar_swords_golden_img), "  radar swords", 0.9, False, 5):
                    ...
                    and '''
                    if find_and_click((radar_skull_blue_img, radar_skull_purple_img, radar_skull_golden_img), "  radar skull", 0.9):
                        
                        if radar_attack() >0:#execute attack
                            break
                    else:
                        character_state[player_str]["radar_skull"]= datetime.now()#no skulls
                        '''if (not "radar_devil" in character_state or character_state["radar_devil"] > datetime.now() + timedelta(days=1)) and''' 
