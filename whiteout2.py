...
def radar(current_player_id):
 ...
 #   time.sleep(2*wait_time)
    ...
    if not character_state[player_str].get("no troops") is None:
        cnt, maximum = read_marches()
        if cnt<maximum:
            character_state[player_str].pop("no troops")
        else:
            print("No troops - skip radar")
            return True
    if not character_state[player_str].get("no stamina") is None:
        cnt = read_stamina()
        if cnt<20:
            print("No troops - skip radar")
            return True
    if not character_state[player_str].get("radar_flag_date") is None and (character_state[player_str]["radar_flag_date"]!=datetime.now(timezone.utc).strftime("%Y%m%d") or character_state[player_str]["radar_flag_part"]!=radar_day_part()):
         #unset the flag
         character_state[player_str].pop("radar_flag_date")
         character_state[player_str].pop("radar_flag_part")

    if not character_state[player_str].get("radar_flag_date") is None:
        print("   Radar on hold")
        return True
    ...
    while True:
        if find_and_click((radar_img,location_radar_img), "  radar") :
            ...
            while find_and_click((radar_tent_gold_done_img, radar_tent_purple_done_img, radar_tent_blue_done_img, radar_swords_blue_done_img, radar_swords_purple_done_img, radar_swords_golden_done_img, radar_skull_blue_done_img, radar_skull_purple_done_img, radar_skull_golden_done_img, radar_devil_done_img), "  radar done", 0.8, False, 2):
                print("   ->Done loop")
            ...
            if find_and_click((radar_tent_blue_img, radar_tent_gold_img, radar_tent_purple_img), "  radar tent", 0.9):
                ...
                if find_and_click((radar_swords_blue_img, radar_swords_purple_img,radar_swords_golden_img), "  radar swords", 0.9, False, 5):
                    ...
                    if character_state[player_str].get("no troops") is None and find_and_click((radar_skull_blue_img, radar_skull_purple_img, radar_skull_golden_img), "  radar skull", 0.9):
                        
                        if radar_attack() >0:#execute attack
                            break
                    else:
                        #only devil is left
                        character_state[player_str]["radar_flag_date"]=datetime.now(timezone.utc).strftime("%Y%m%d")
                        
                        character_state[player_str]["radar_flag_part"]=radar_day_part()

                        if find_and_click((radar_devil_img,), "  radar devil"):
                            
                            ret = radar_attack()
                            if ret == 1 : # no troops available or no stamina
                                character_state[player_str]["no troops"]=True
                                break
                            elif ret == 3:
                                character_state[player_str]["no stamina"]=True
                                break
...
def pet_adventure():
    ...
    if find_and_click((left_bar_img, left_bar2_img), "   Left bar", 0.7):
        ...
        if find_and_click((pet_adventure_ready_img, pet_adventure_completed_img), "   pet adventure ready left bar", 0.7, False):
            ...
            while find_and_click((pet_adventure_medium_done_img, pet_adventure_low_done_img), "   pet adventure done", 0.9, False):
                find_and_click((pet_adventure_done_collect_medium_img, pet_adventure_done_collect_low_img, pet_adventure_done_collect_high_img), "   pet adventure collect", 0.7, False, 20)
                while find_and_click((pet_adventure_collect_tap_img,), "   pet adventure collect tap", 0.7, False, 30):
                    time.sleep(wait_time*0.2)
            for i in range(3):
                if find_and_click((pet_adventure_high_img, pet_adventure_medium_img, pet_adventure_low_img,), "  pet adventure chest", 0.9, False):
                    if find_and_click((pet_adventure_select_pet_img,), "   select pet", 0.7, False):
                        if not find_and_click((pet_adventure_start_img,), "   pet adventure start", 0.7, False):#no more tries
