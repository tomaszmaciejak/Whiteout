...
def find_image(images, debug_name="", threshold=0.8, important=False, cnt_tries=5):
    ...
    for image in images:
        ...
        for i in range(cnt_tries):
            ...
            screenshot_b, screenshot_g, screenshot_r = cv2.split(screenshot)

            # print(f"image type={type(image)}")
            # Perform template matching
            image_b, image_g, image_r = cv2.split(image)
            result_r = cv2.matchTemplate(screenshot_r, image_r, cv2.TM_CCOEFF_NORMED)
            result_g = cv2.matchTemplate(screenshot_g, image_g, cv2.TM_CCOEFF_NORMED)
            result_b = cv2.matchTemplate(screenshot_b, image_b, cv2.TM_CCOEFF_NORMED)
            ...
            if debug > 0 or important:
                ...
                if max_val >= threshold:
                    t = t + " OK"
                    print("R=" + str(result_r[y, x]) + " G=" + str(result_g[y, x]) + " B=" + str(result_b[y, x]))

                debug_screenshot = copy.deepcopy(screenshot)
                debug_screenshot[180:180 + image.shape[0], 0:image.shape[1]] = image
                debug_screenshot = cv2.putText(debug_screenshot, t, (0, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0),
                                               2, cv2.LINE_AA)  # fit
    ...
    if find_and_click((exploration_img, exploration2_img), "   Exploration"):
        ...
        if find_and_click((exploration_claim_img,), "   Claim in Exploration", 0.9, False, 5):
            ...
            if find_and_click((exploration_claim_confirm_img,), "   Confirm Claim in Exploration", 0.9, False, 5):
                ...
                #character_state[player_id]["exploration"] = datetime.now()
...
def train_troops(update_only=False):
    ...
    while any_change:
        ...
        if find_and_click((left_bar_img, left_bar2_img), "   Left bar", 0.7):
            time.sleep(wait_time)
            swipe(915, 474, 915, 374)
            if find_and_click((marksman_completed_img, marksman_idle_img), "   marksman completed or idle in a bar",
                              0.9, False, 1):
                train_marksman(update_only)
                any_change = True
            elif find_and_click((infantry_completed_img, infantry_idle_img), "   infantry completed or idle in a bar",
                                0.9, False, 1):
                train_infantry(update_only)
                any_change = True
            elif find_and_click((lancers_completed_img, lancers_idle_img), "   lancers completed or idle in a bar", 0.9,
                                False, 1):
    ...
    if find_and_click((view_img,), "  view"):
        ...
        if find_and_click((attack_img,), "  attack"):
            ...
            if find_and_click((march_queue_img,), "  march queue"):  # no troops
                ...
                return 1
            if find_image((attack_likely_to_prevail_img, attack_risky_img), "  attack likely to prevail") != (
            None, None):  # not too strong opponent
...
def radar_next_hour():
    ...
    if hour < 8:
        ...
        return datetime(date.year, date.month, date.day, 16)
    else:
        return datetime(date.year, date.month, date.day, 0)+timedelta(days=1)
...
def radar():
    ...
    # devil_visited = False
    if not character_state[player_id].get("radar:next_attempt") is None:
        if datetime.now()<character_state[player_id].get("radar:next_attempt"):
            print("   Radar on hold")
            return True #skip
        else:
            character_state[player_id].pop("radar:next_attempt")

    if not character_state[player_id].get("radar:no_troops") is None:
        cnt, maximum = read_marches()
        if cnt < maximum:
            character_state[player_id].pop("radar:no_troops")
        else:
            print("No troops - skip radar")
            return True

    if not character_state[player_id].get("global:no_stamina") is None:
        cnt = read_stamina()
        if cnt < 20:#stamina below 20
            print("No troops - skip radar")
            return True
        else:
            character_state[player_id].pop("global:no_stamina")

    if not character_state[player_id].get("radar:no_tasks") is None:
        character_state[player_id].pop("radar:no_tasks") #if time passed turn it off
    ...
    while True:
        if find_and_click((radar_img, location_radar_img), "  radar"):
            ...
            while find_and_click((radar_tent_gold_done_img, radar_tent_purple_done_img, radar_tent_blue_done_img,
                                  radar_swords_blue_done_img, radar_swords_purple_done_img,
                                  radar_swords_golden_done_img, radar_skull_blue_done_img, radar_skull_purple_done_img,
                                  radar_skull_golden_done_img, radar_devil_done_img), "  radar done", 0.8, False, 2):
                print("   ->Done loop")
            ...
            if find_and_click((radar_tent_blue_img, radar_tent_gold_img, radar_tent_purple_img), "  radar tent", 0.9):

                find_and_click((radar_claim_img,), "  tent claim", 0.9, False,
                               5)  # tent in progress and completed when on
                ...
                if find_and_click((radar_swords_blue_img, radar_swords_purple_img, radar_swords_golden_img),
                                  "  radar swords", 0.9, False, 5):
                    ...
                    if find_and_click((view_img,), "  view"):
                        ...
                        if find_and_click((radar_explore_img,), "  explore"):
                            ...
                            if find_and_click((fight_img,), "  fight", 0.7, False, 30):  # normal fight
                                ...
                                # normal ending
                            # stamina_missing
                            elif find_image((stamina_missing_img,), "   stamina_missing") != (None, None):
                                find_and_click((stamina_missing_close_img,), "   stamina_missing_close")
                                find_and_click((exploration_back_img,), "   exploration_back")
                                break
                    else:
                    ...
                    if character_state[player_id].get("radar:no_troops") is None and find_and_click(
                            (radar_skull_blue_img, radar_skull_purple_img, radar_skull_golden_img), "  radar skull",
                            0.9):
                        ...
                        # only devil is left
                        character_state[player_id]["radar:no_tasks"] = True

                        character_state[player_id]["radar:next_attempt"] = radar_next_hour()

                        if find_and_click((radar_devil_img,), "  radar devil"):

                            ret = radar_attack()
                            if ret == 1:  # no troops available or no stamina
                                character_state[player_id]["radar:no_troops"] = True
                                break
                            elif ret == 3:
                                character_state[player_id]["global:no_stamina"] = True
                                break
                            elif ret == 2:  # too strong
...
def first_screen():
    ...
    if find_and_click((add_cross_img, add_cross2_img, add_cross3_img, add_cross4_img, add_cross5_img), "...add cross",
                      0.8, False, 50) != (None, None):
...
def reset_location():
    ...
    while find_image((location_infantry_training_img, location_lancer_training_img, location_marksman_training_img),
                     "...training location") != (None, None):
...
def change_player(current_player_id):
    ...
    if find_and_click((current_player,), "   enter profile", 0.7, False):
        ...
        find_and_click((target_player,), "   select player 2", 0.7, False)
...
def alliance_tech():
    ...
    if not character_state[player_id].get("alliance_tech:next_attempt") is None:
        if character_state[player_id].get("alliance_tech:next_attempt")>datetime.now():
            ...
            character_state[player_id].pop("alliance_tech:next_attempt")
    ...
    if find_and_click((left_bar_img, left_bar2_img), "   Left bar", 0.7):
        ...
        if find_and_click((alliance_contribution_img,), "   alliance_contribution", 0.7, False, 1):
            ...
            if not find_and_click((alliance_tech_img,), "   alliance_tech", 0.7, False, 10):
                return None
            if find_and_click((alliance_tech_recommended_img,), "   alliance_tech recommended", 0.7):
                try_number = 1
                while find_image((alliance_cannot_contribute_img,), "   alliance cannot contribute") == (
                None, None) and try_number <= 25:
                ...
                if find_and_click((alliance_chests_gift_claim_all_img,), "   alliance chests gift claim all", 0.95):
                    find_and_click((alliance_chests_tap_img,), "   alliance chests tap", 0.7)
                if find_and_click((alliance_chests_loot_img,), "   alliance chests loot", 0.7):
                    if find_and_click((alliance_chests_claim_all_img,), "   alliance chests claim all", 0.95):
                        find_and_click((alliance_chests_tap_img,), "   alliance chests tap", 0.7)
            find_and_click((alliance_tech_back_img,), "   alliance_tech back", 0.7)
            find_and_click((alliance_tech_back_img,), "   alliance_tech back", 0.7)
            character_state[player_id]["alliance_tech:next_attempt"] = datetime.now()+timedelta(hours=1)
        else:
            find_and_click((close_bar_img,), "   close bar", 0.5)
...
def backpack():
    ...
    if find_and_click((backpack_img,), "   backpack"):
        ...
        while any_change:
            any_change = False
            if find_and_click((backpack_diamonds_1k_img, backpack_diamonds_1_img, backpack_diamonds_10_img,
                               backpack_diamonds_100_img, backpack_exp_1k_img, backpack_exp_5k_img,
                               backpack_exp_10k_img, backpack_exp_50k_img), "   backpack resources", 0.7, False):
                any_change = True
...
def tree_of_life():
    ...
    if not character_state[player_id].get("tree:next_attempt") is None:
        if character_state[player_id].get("tree:next_attempt")>datetime.now():
            ...
            character_state[player_id].pop("tree:next_attempt")
    ...
    if find_and_click((left_bar_img, left_bar2_img), "   Left bar", 0.7):
        ...
        if find_and_click((tree_img,), "   tree left bar", 0.7, False):
            while find_and_click((tree_collect_img,), "   tree collect", 0.7, False, 20) and cnt_tries < max_tries:
                time.sleep(0.2 * wait_time)
                cnt_tries = cnt_tries + 1
            find_and_click((tree_close_img,), "   tree close", 0.7, False, 20)
            find_and_click((tree_exit_img,), "   tree exit", 0.7, False, 20)
            character_state[player_id]["tree:next_attempt"] = datetime.now()+timedelta(hours=1)
...
def pet_adventure():
    ...
    if not character_state[player_id].get("global:no_stamina") is None:
        cnt = read_stamina()
        if cnt < 10:#stamina below 10
            print("No troops - skip pets")
            return True
        else:
            character_state[player_id].pop("global:no_stamina")

    if not character_state[player_id].get("pets:no_tasks") is None:
        character_state[player_id].pop("pets:no_tasks") #if time passed turn it off
    ...
    if find_and_click((left_bar_img, left_bar2_img), "   Left bar", 0.7):
        ...
        swipe(915, 474, 915, 374)
        time.sleep(wait_time)
        if find_and_click((pet_adventure_ready_img, pet_adventure_completed_img), "   pet adventure ready left bar",
                          0.7, False):
            #ally's treasures
            if find_and_click((pet_adventure_ally_img,), "   pet adventure ally", 0.95, False):
                find_and_click((pet_adventure_ally_collect_all_img,), "   pet adventure ally collect all", 0.7, False)
                find_and_click((pet_adventure_ally_collect_tap_img,), "   pet adventure ally collect tapo", 0.7, False,
                               20)
                find_and_click((pet_adventure_ally_collect_exit_img,), "   pet adventure ally collect exit", 0.7, False,
                               20)

            #pet collect completed
            while find_and_click((pet_adventure_high_done_img, pet_adventure_medium_done_img, pet_adventure_low_done_img), "   pet adventure done",
                                 0.9, False):
                find_and_click((pet_adventure_done_collect_medium_img, pet_adventure_done_collect_low_img,
                                pet_adventure_done_collect_high_img), "   pet adventure collect", 0.7, False, 20)
                while find_and_click((pet_adventure_collect_tap_img,), "   pet adventure collect tap", 0.7, False, 30):
                    time.sleep(wait_time * 0.2)

            #start new treasure hunt
            for i in range(3):
                if find_and_click((pet_adventure_high_img, pet_adventure_medium_img, pet_adventure_low_img,),
                                  "  pet adventure chest", 0.9, False):
                    if find_and_click((pet_adventure_select_pet_img,), "   select pet", 0.7, False):
                        if not find_and_click((pet_adventure_start_img,), "   pet adventure start", 0.7,
                                              False):  # no more tries
                            #no tasks - try next day
                            character_state[player_id]["pets:no_tasks"] = True
                            character_state[player_id]["pets:next_attempt"] = pets_next_time()
                            while find_and_click((pet_adventure_start_exit_img,), "   pet adventure start exit", 0.7,
                                                 False, 20):
                                find_and_click((pet_adventure_start_exit_img,), "   pet adventure start exit", 0.7,
                                               False, 20)
                            break
                    else: #TODO - no stamina?
                        character_state[player_id]["global:no_stamina"] = True
                        character_state[player_id]["pets:next_attempt"] =datetime.now()+timedelta(hours=1) #TODO it should be configurable
...
def serialize_datetime(obj):
    ...
    raise TypeError("Type is nor serializable")


def read_stamina():
    print("Read stamina")
    if player_id == 1:  # current player ID
        current_player = player_1_img
    ...
    if find_and_click((current_player,), "   enter profile", 0.7, False):
        x, y = find_image([chief_profile_stamina_img, ], "Chief profile - stamina", 0.7, False)
        screenshot = pyautogui.screenshot()
        # Convert to OpenCV format
        screenshot = np.array(screenshot)
        stamina_shot = screenshot[y - 25:y + 25, x:x + 100]

        stamina_shot = cv2.cvtColor(stamina_shot, cv2.COLOR_RGBA2BGR)
        result = OCRreader.readtext(stamina_shot, detail=0)
        print(result[0])
        i = result[0].find("/")
        if debug > 0:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"debug/{timestamp}_stamina.png"
            cv2.imwrite(filename, stamina_shot)
        find_and_click((chief_profile_exit_img,), "   chief_profile_exit", 0.7, False)

        return result[0][:i]
    else:
        print("ERROR - cannot enter profile")
        return -1
...
def read_marches():
    ...
    x, y = find_image([marches_img, ], "Marches", 0.7, False)
    screenshot = pyautogui.screenshot()
    screenshot = np.array(screenshot)
    stamina_shot = screenshot[y - 25:y + 25, x + 50:x + 100]
    ...
    if debug > 0:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"debug/{timestamp}_marches.png"
        cv2.imwrite(filename, stamina_shot)
        print(result[0][:i], result[0][i + 1:])
    return result[0][:i], result[0][i + 1:]
...
chief_profile_stamina_img = load_image("icons/chief_profile_stamina.png")
marches_img = load_image("icons/marches.png")
chief_profile_exit_img = load_image("icons/chief_profile_exit.png")
...
tree_close_img = load_image("icons/tree_of_life_close.png")
...
pet_adventure_high_done_img = load_image("icons/pet_adventure_high_done.png")
...
pet_adventure_done_collect_medium_img = load_image("icons/pet_adventure_done_collect_medium.png")
pet_adventure_done_collect_low_img = load_image("icons/pet_adventure_done_collect_low.png")
pet_adventure_done_collect_high_img = load_image("icons/pet_adventure_done_collect_high.png")
...
'''
character_state = {1: {"player_id": 1}, 2: {"player_id": 2}}
# debug=1
# tree_of_life()
# pet_adventure()
# alliance_tech()
