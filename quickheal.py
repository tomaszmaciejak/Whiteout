 ...
 #   time.sleep(2*wait_time)
    ...
    while True:
        if find_and_click((radar_img,location_radar_img), "  radar") :
            ...
            time.sleep(wait_time)
            if (not "radar_done" in character_state or character_state["radar_done"] > datetime.now() + timedelta(hours=8)) or (not "radar_tent" in character_state or character_state["radar_tent"] > datetime.now() + timedelta(hours=8)) or (not "radar_swords" in character_state or character_state["radar_swords"] > datetime.now() + timedelta(hours=8)) or (not "radar_skull" in character_state or character_state["radar_skull"] > datetime.now() + timedelta(hours=8)) or (not "radar_devil" in character_state or character_state["radar_devil"] > datetime.now() + timedelta(days=1)):
            ...
            character_state["radar_done"]= datetime.now()
            if (not "radar_tent" in character_state or character_state["radar_tent"] > datetime.now() + timedelta(hours=8)) and find_and_click((radar_tent_blue_img, radar_tent_gold_img, radar_tent_purple_img), "  radar tent", 0.9):
                #time.sleep(wait_time)
                ...
                if (find_and_click((view_img,), "  view")):
                    ...
                    #time.sleep(wait_time)
                    if (find_and_click((radar_tap_anywhere_img,), "  click anywhere", 0.7)):
                        break
            else:
                character_state["radar_tent"]= datetime.now()
                if (not "radar_swords" in character_state or character_state["radar_swords"] > datetime.now() + timedelta(hours=8)) and find_and_click((radar_swords_blue_img, radar_swords_purple_img,radar_swords_golden_img), "  radar swords", 0.9, False, 5):
                    #time.sleep(wait_time)
                    ...
                    if (find_and_click((view_img,), "  view")):
                        ...
                        if (find_and_click((radar_explore_img,), "  explore")):
                            #time.sleep(wait_time)
                            if (find_and_click((fight_img,), "  fight"), 0.7, False, 30):
                                #time.sleep(5*wait_time)
                                find_and_click((fight_tap_anywhere_img,), "  fight - tap anywhere", 0.7, False, 25)
                            else: #stamina_missing
                                #time.sleep(wait_time)
                                if (find_image((stamina_missing_img,), "   stamina_missing", False, 30) != (None, None)):
                                    #time.sleep(wait_time)
                                    find_and_click((stamina_missing_close_img,), "   stamina_missing_close")
                                    #time.sleep(wait_time)
                                    find_and_click((exploration_back_img,), "   exploration_back")
                    ...
                    character_state["radar_swords"]= datetime.now()
                    if (not "radar_skull" in character_state or character_state["radar_skull"] > datetime.now() + timedelta(hours=8)) and find_and_click((radar_skull_blue_img, radar_skull_purple_img, radar_skull_golden_img), "  radar skull", 0.9):
                        #time.sleep(wait_time)
                        if radar_attack() >0:
                            break
                    else:
                        character_state["radar_skull"]= datetime.now()
                        if (not "radar_devil" in character_state or character_state["radar_devil"] > datetime.now() + timedelta(days=1)) and find_and_click((radar_devil_img,), "  radar devil"):
                            ...
                            if ret == 1 or ret == 3: # no troops available or no stamina
                                character_state["no troops"]=datetime.now()
                                break
                            elif ret == 2:# too strong - TODO: it can be because main march is unavailable
                                character_state["radar_devil"]= datetime.now()
                                devil_visited=True
                                break
                        else:
                            return False
...
def first_screen():
    ...
    if find_image((add_cross_img, add_cross3_img, add_cross4_img, add_cross5_img, add_cross6_img), "...add cross", 0.7, False, 50)!= (None, None):
...
def backpack():
    ...
    if find_and_click((backpack_img,), "   backpack"):
        ...
        while any_change:
            any_change=False
            if find_and_click((backpack_diamonds_1k_img, backpack_diamonds_1_img, backpack_diamonds_10_img, backpack_diamonds_100_img, backpack_exp_1k_img, backpack_exp_5k_img, backpack_exp_10k_img), "   backpack resources", 0.7, False):
                any_change=True
...
def tree_of_life():
    ...
    if find_and_click((left_bar_img, left_bar2_img), "   Left bar", 0.7):
        ...
        if find_and_click((tree_img,), "   tree left bar", 0.7, False):
            while find_and_click((tree_collect_img, ),"   tree collect", 0.7, False, 20) and cnt_tries<20:
                time.sleep(0.2*wait_time)
...
def pet_adventure():
    ...
    if find_and_click((left_bar_img, left_bar2_img), "   Left bar", 0.7):
        ...
        if find_and_click((pet_adventure_ready_img, pet_adventure_completed_img), "   pet adventure ready left bar", 0.7, False):
            while find_and_click((pet_adventure_medium_done_img,), "   pet adventure done", 0.7, False):
                find_and_click((pet_adventure_done_collect_img,), "   pet adventure collect", 0.7, False, 20)
                while find_and_click((pet_adventure_done_collect_tap_img,), "   pet adventure collect tap", 0.7, False, 30):
            ...
            for i in range(3):
                if find_and_click((pet_adventure_medium_img, pet_adventure_low_img,), "  pet adventure chest", 0.7, False):
...
add_cross6_img = load_image ("icons/add_cross6.png")
